...
/**************************************************
 * Breeze background speed control
 **************************************************/
function initBreeze() {
  const breezeBg = document.querySelector(".breeze-bg");
  if (!breezeBg) return;
  ...
  function tick(now) {
    ...
    const mapped = 1 + (velocity / 1200) * (MAX - 1);
    targetSpeed = clamp(mapped, MIN, MAX);
    if (dy < 0.5) targetSpeed = Math.max(1, currentSpeed * DECAY);
    currentSpeed = lerp(currentSpeed, targetSpeed, RESPONSE);
    root.style.setProperty("--breeze-speed", currentSpeed.toFixed(3));
    lastY = y;
    lastT = now;
